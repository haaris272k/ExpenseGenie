...
def start(message):
    ...
    try:
        ...
        # Checking if username is None then set the username to user_id
        if username is None:
            ...
            bot.reply_to(
                message,
                f"Good to see you. With the BudgetWizardBot , you can easily keep track of all your expenses."
            ...
            bot.reply_to(
                ...
                " Please set a username in the <b>settings</b> section of your telegram account for a better experience."
        ...
        if dbobj.collection.find_one({"u_id": user_id}):
            # Wait
            time.sleep(1)
...
def add_expense(message):
    ...
    """
    This function will add the expense to the database corresponding to the unique user ID.
...
def get_tag(message, dbobj, username, user_id, expense):
    ...
    try:
        ...
        expense_data = {
            "username": username if username is not None else "user" + str(user_id),
            "u_id": user_id,
            "expense": expense,
            "tag": tag,
            "timestamp": timestamp,
...
def view_expense(message):
    ...
    # Getting the data from the database corresponding to the user ID
    query = {"u_id": user_id}
...
def total_expense(message):
    ...
    bot.register_next_step_handler(message, get_choice, dbobj, user_id)


def get_choice(message, dbobj, user_id):
...
def delete_expense(message):
    ...
    # Waiting for the user to enter the choice
    bot.register_next_step_handler(message, get_delete_choice, dbobj, user_id)


def get_delete_choice(message, dbobj, user_id):
    ...
    try:
        ...
        if choice == "1":
            ...
            bot.register_next_step_handler(message, get_id, dbobj, user_id)

    except:
        bot.reply_to(message, "There was some error.")


def get_id(message, dbobj, user_id):
